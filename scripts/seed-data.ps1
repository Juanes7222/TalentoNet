...
foreach ($seedFile in $seedFiles) {
    ...
    if (Test-Path $seedFile) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  ⚠️ $fileName ejecutado con advertencias (puede ser que ya existan datos)" -ForegroundColor Yellow
            $seedErrors++
        }
    } else {
...
if ($seedSuccess -eq $totalSeeds) {
    ...
    Write-Host "⚠️ Seeds ejecutados parcialmente" -ForegroundColor Yellow
